...
<!-- Version: $Id: diffspec.xsl,v 1.4 2013-10-02 14:34:04 NormanWalsh Exp $ -->
...
div.diff-del  { text-decoration: line-through; background-color: #FF9999; }
div.diff-chg  { background-color: #99FF99; }
div.diff-off  {  }

span.diff-add { background-color: #FFFF99; }
span.diff-del { text-decoration: line-through; background-color: #FF9999; }
span.diff-chg { background-color: #99FF99; }
span.diff-off {  }

td.diff-add   { background-color: #FFFF99; }
td.diff-del   { text-decoration: line-through; background-color: #FF9999; }
    ...
    <span>
      <xsl:if test="@diff = 'del'">
        <xsl:attribute name="class">
          <xsl:text>diff-del</xsl:text>
        </xsl:attribute>
      </xsl:if>
      <xsl:apply-templates select="." mode="divnum"/>
      <!-- If this div was deleted, don't emit a (broken) link
           when not showing diff markup
           and do emit some extra text (pbg 2013 Sep 17) -->
      <xsl:choose>
        <xsl:when test="@diff='del' and $show.diff.markup = 0">
            <xsl:apply-templates select="head" mode="text"/>
            <i> [This section has been deleted.]</i>
        </xsl:when>
        <xsl:otherwise>
          <a>
